...
← {"id":1,"result":{"protocol":2,"columns":true,"indexes":true,"ddl":true,"readonly":true,"fkeys":true}}
```

| method | params | 说明 |
|---|---|---|
| `open` | `source.Config`（driver/dsn/ssh） | 建连接；result 含协议版本与五个能力位 |
| `ping` | — | 探活 |
...
| `columns` / `indexes` / `ddl` / `foreign_keys` | `{"path":["db","table"]}` | 仅 `open` 对应能力位为 true 时可用 |
| `close` | — | 关连接；之后进程可退出 |
...
1. 实现接口（`internal/source`）：
   ...
   - `ColumnLister`/`IndexLister`/`DDLShower`/`ReadOnlyExecer`/`ForeignKeyLister`（可选，能力接口按需实现）
